...
internal sealed class TimeMarker : FlagSet
{
	...
	public override List<Flag> FlagsList { get; } = new();
	...
	public override string Process(string line)
	{
		...
		switch (_timeDeltaMode) {
			case TimeDeltaMode.PerThread: {
				...
				getLast = () => _lastPerThreadTimes.ContainsKey(threadID)
					? _lastPerThreadTimes[threadID]
					: DateTime.MinValue;
		...
		return _outputTimeSpan
			...
			: string.Concat(prefix, pad(new string(markOn, onSize), _size, markOff), SEPARATOR);
	}
